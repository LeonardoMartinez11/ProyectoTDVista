...
<div>
    ...
    <p><strong>Fecha Inicio:</strong> <?= isset($viaje['fecha_inicio']) ? date('d/m/Y H:i', strtotime($viaje['fecha_inicio'])) : '' ?></p>
    <p><strong>Fecha Fin actual:</strong> <?= isset($viaje['fecha_fin']) ? date('d/m/Y H:i', strtotime($viaje['fecha_fin'])) : 'dd/mm/aaaa --:--' ?></p>
    <p><strong>Ingreso Total:</strong> <?= htmlspecialchars($viaje['ingreso_total'] ?? 0) ?></p>
    <p><strong>Pago Acordado al Chofer:</strong> <?= htmlspecialchars($viaje['pago_acordado_chofer'] ?? 0) ?></p>
</div>
...
<form method="POST">
    <label>Kilómetros Recorridos:
        <input type="number" name="kilometros_recorridos" value="<?= htmlspecialchars($viaje['kilometros_recorridos'] ?? 0) ?>">
    </label><br>

    <label>Combustible Usado (G):
        <input type="number" step="0.01" name="combustible_usado" value="<?= htmlspecialchars($viaje['combustible_usado'] ?? 0) ?>">
    </label><br>

    <label>Fecha Fin (editar):
        <input type="datetime-local" name="fecha_fin" value="<?= isset($viaje['fecha_fin']) ? date('Y-m-d\TH:i', strtotime($viaje['fecha_fin'])) : '' ?>">
    </label><br>
    ...
    <label>Observaciones:<br>
        <textarea name="observaciones" rows="4" cols="50"><?= htmlspecialchars($viaje['observaciones'] ?? '') ?></textarea>
    </label><br>
...
<table border="1" cellpadding="6">
    <tr>
        <th>#</th><th>Tipo</th><th>Descripción</th><th>Monto</th><th>Fecha</th>
    </tr>
    ...
    <tr>
        ...
        <td><?= htmlspecialchars($g['monto'] ?? 0) ?></td>
